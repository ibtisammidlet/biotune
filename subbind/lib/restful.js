...
app.get('/', (req, res) => { // Handle incoming GET requests to http://localhost:(port)/
    res.sendFile(path.join(__dirname + '/restful.html')); // Send the index.html file
});
...
app.post('/subbind', (req, res) => { // Will run when there are any incoming POST requests to http://localhost:(port)/user. Note that a POST request is different from a GET request, so this won't exactly work when you actually visit http://localhost:(port)/user
    ...
    data_1.append('scope', 'identity'); // This tells the Discord API what info you would like to retrieve. You can change this to include guilds, connections, email, etc.
...
console.log(data_1)
    ...
    axios.get("https://discord.com/api/users/@me", make_config(data.access_token)).then(response => { // Make a request yet again to the Discord API with the token from previously.
            ...
            ", avatar:"+"https://cdn.discordapp.com/avatars/"+response.data.id+"/"+response.data.avatar+".png"); // Send the username with a status code 200.
